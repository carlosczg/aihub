...
`converters.py`). `.md` sources are passed through with their own leading
...
`source_metadata`) followed by the body. The front matter deliberately
carries **no timestamp and no run identifier** — output is byte-identical
for the same source bytes and the same converter version, regardless of
when or in which run it was produced. Body content and line endings after
any detected source front matter are preserved verbatim.
